...
export function Dashboard() {
  ...
  const report = useQuery({
    ...
    enabled: !!tenantId && session?.plan !== "FREE",
  });
  ...
  const commercialDashboard = session.plan !== "FREE";
  const copy = () =>
    navigator.clipboard
  ...
  return (
    <>
      ...
      {!commercialDashboard ? (
        <Card className="border-teal-200 bg-gradient-to-br from-white to-teal-50 py-10">
          <TrendingUp className="text-teal-700" size={32} />
          <p className="mt-5 text-sm font-bold uppercase tracking-widest text-teal-700">
            Rendimiento comercial
          </p>
          <h1 className="mt-2 text-3xl font-semibold text-teal-950">
            Descubre cómo está creciendo tu negocio
          </h1>
          <p className="mt-3 max-w-2xl leading-7 text-slate-600">
            Desde el plan Basic podrás consultar el total de reservas, ingresos
            potenciales, ingresos completados y el desglose por estado.
          </p>
          <Link
            href="/admin/plan"
            className="mt-6 inline-flex min-h-11 items-center rounded-xl bg-slate-950 px-5 text-sm font-bold text-white transition hover:bg-teal-800"
          >
        ...
        <>
          <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
            ...
            <Button
              onClick={exportExcel}
              className={
                !commercialTools ? "bg-slate-400 hover:bg-slate-400" : ""
              }
            >
              ...
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <Metric
                  ...
                  value={
                    commercialTools
                      ? String(
                          data?.customers.filter((x) => x.recurring).length ??
                            0,
                        )
                      : "Plan Plus"
                  }
