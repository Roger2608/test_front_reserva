...
export function DesignEditor() {
  ...
  const branding = useQuery({
    ...
    queryFn: () => api<Branding>("/api/v1/tenant/branding"),
    enabled: !!tenantId,
  });
  ...
  if (branding.isLoading || subscription.isLoading)
    return <Card>Cargando editor…</Card>;
  return (
    <DesignForm
      key={`${tenantId}-${branding.data?.version}`}
      initial={{ ...defaults, ...branding.data }}
      enabled={
        subscription.data?.canCustomizeDesign ??
        session?.canCustomizeDesign ??
        false
      }
      premium={subscription.data?.premiumDesign ?? session?.plan === "PREMIUM"}
      tenantName={session?.tenant?.name ?? "Tu empresa"}
      tenantId={tenantId}
    />
  );
}
...
function DesignForm({
  ...
  const font =
    form.fontPreset === "MODERN"
      ? 'Inter,"Segoe UI",sans-serif'
      : form.fontPreset === "FRIENDLY"
        ? 'Nunito,"Segoe UI",sans-serif'
        : "Georgia,serif";
  const save = useMutation({
    mutationFn: () =>
      api<Branding>("/api/v1/tenant/branding", {
        method: "PUT",
        ...
        <form className="mt-6 grid gap-4" onSubmit={submit}>
          ...
          <Field label="Tipografía">
            <Select
              ...
              onChange={(e) =>
                set("fontPreset", e.target.value as Form["fontPreset"])
              }
            >
          ...
          <Field label="Composición Premium">
            <Select
              disabled={!premium}
              value={form.themePreset}
              onChange={(e) =>
                set("themePreset", e.target.value as Form["themePreset"])
              }
            >
          ...
          <Field label="Botones Premium">
            <Select
              disabled={!premium}
              value={form.buttonStyle}
              onChange={(e) =>
                set("buttonStyle", e.target.value as Form["buttonStyle"])
              }
            >
          ...
          <Button disabled={!enabled || save.isPending}>
            {save.isPending ? "Publicando…" : "Publicar diseño"}
          </Button>
        </form>
      </Card>
      <div
          ...
          style={
            {
              ...
              backgroundImage: form.coverUrl
                ? `linear-gradient(#0008,#0008),url(${form.coverUrl})`
                : undefined,
        ...
        >
          {form.logoUrl && (
            <span
              ...
              className="mb-8 block h-12 w-48 bg-contain bg-left bg-no-repeat"
          ...
          <h2 className="mt-4 text-5xl font-semibold">
            {form.heroTitle || tenantName}
          </h2>
          <p className="mt-4 max-w-lg text-lg opacity-90">
            {form.heroDescription ||
              "Elige el servicio y encuentra un horario disponible."}
          </p>
          <button
            type="button"
            className={`mt-8 px-5 py-3 font-bold text-slate-950 ${form.buttonStyle === "PILL" ? "rounded-full" : form.buttonStyle === "SQUARE" ? "rounded-none" : "rounded-xl"}`}
