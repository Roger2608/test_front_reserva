...
export function AdvertisementDirectory() {
  const campaigns = useQuery({
    queryKey: ["advertisement-directory"],
    queryFn: () => api<Advertisement[]>("/public/v1/advertisements/directory"),
  });
  const [featured, setFeatured] = useState(0);
  useEffect(() => {
    if (!campaigns.data || campaigns.data.length < 2) return;
    const timer = window.setInterval(
      () => setFeatured((value) => (value + 1) % campaigns.data!.length),
      6000,
    );
      ...
      <section className="bg-teal-950 py-16 text-white">
        <div className="shell">
          ...
          <h1 className="mt-4 max-w-3xl text-5xl font-semibold">
            Descubre empresas que tienen algo valioso para ofrecerte
          </h1>
          <p className="mt-4 max-w-2xl text-lg text-teal-100">
            Campañas activas de negocios Premium de nuestra comunidad,
            organizadas para conectar con nuevos clientes.
          </p>
        </div>
      </section>
      <div className="shell py-12">
        {campaigns.isLoading ? (
          <Card>Cargando campañas…</Card>
        ) : !campaigns.data?.length ? (
          <Empty
            title="Próximamente habrá campañas"
            description="Los negocios Premium aparecerán aquí cuando publiquen sus anuncios."
          />
        ) : (
          <>
            <section>
              ...
              <AdvertisementBanner
                advertisement={
                  campaigns.data[featured % campaigns.data.length]!
                }
              />
            </section>
