...
> = {
  FREE: {
    name: "Gratis",
    description: "Agenda y reservas para comenzar.",
    features: [
      "Horarios y reservas",
      "Anuncios en reserva y panel",
      "Sin automatización WhatsApp",
    ],
  },
  BASIC: {
    name: "Básico",
    description: "Una experiencia limpia para tus clientes.",
    features: [
      "Clientes sin anuncios",
      "Anuncios solo en panel",
      "Sin automatización WhatsApp",
    ],
  },
  PLUS: {
    name: "Plus",
    description: "Marca propia y confirmaciones automáticas.",
    features: [
      "Sin anuncios",
  ...
  PREMIUM: {
    name: "Premium",
    description: "Automatización y promoción para crecer.",
    features: [
      "Confirmación y recordatorio",
      "Publica tus anuncios",
      "Vitrina pública de campañas",
    ],
  },
};
...
export function PlanSelector() {
  ...
  const subscription = useQuery({
    queryKey: ["subscription", session?.tenant?.id],
  ...
  const downgrade = useMutation({
    mutationFn: () =>
      api<void>("/api/v1/payments/change-to-free", { method: "POST" }),
    onSuccess: async () => {
      await refresh();
      setConfirmFree(false);
      toast.success("Tu plan ahora es Gratis");
    },
    onError: (error: Error) => toast.error(error.message),
  });
  ...
  const priceFor = (plan: Plan) =>
    prices.data?.find((item) => item.plan === plan);
  ...
  return (
    <>
      ...
      {subscription.isLoading || prices.isLoading ? (
        <Card aria-live="polite">Cargando planes…</Card>
      ) : subscription.isError || prices.isError ? (
        ...
        <>
            ...
            <Card className="mb-6 flex flex-col items-start justify-between gap-4 border-teal-200 bg-teal-50 sm:flex-row sm:items-center">
              <div>
                ...
                <p className="mt-2 font-semibold text-slate-950">
                  Cambio al plan {details[pending.plan].name}
                </p>
                <p className="text-sm text-slate-600">
                  Puedes continuar el pago o cancelarlo para elegir otro plan.
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  ...
                  onClick={() => cancelCheckout.mutate(pending.id)}
                >
          ...
          {!owner && (
            <Card className="mb-6 flex gap-3 bg-slate-50">
              <LockKeyhole className="shrink-0 text-slate-500" />
              <p className="text-sm text-slate-600">
                Solo el propietario de la empresa puede cambiar el plan. Puedes
                consultar aquí sus beneficios y estado.
              </p>
            </Card>
          )}
              ...
              return (
                ...
                >
                  <div className="flex min-h-7 items-center justify-between">
                    {plan === "PLUS" ? (
                      <span className="flex items-center gap-1 text-xs font-bold uppercase tracking-wider text-teal-700">
                        <Sparkles size={14} />
                        Recomendado
                      </span>
                    ) : (
                    ...
                    {active && <Badge tone="green">PLAN ACTUAL</Badge>}
                  </div>
                  ...
                  <ul className="my-6 grid gap-3 text-sm text-slate-700">
                    {details[plan].features.map((feature) => (
          ...
          {confirmFree && (
            ...
            >
              ...
              <p className="mt-2 text-sm leading-6 text-slate-700">
                El cambio es inmediato. Tu página y reservas seguirán activas,
                pero perderás las opciones de personalización visual de Plus o
                Premium.
              </p>
              <div className="mt-5 flex flex-wrap gap-3">
                ...
                >
                  {downgrade.isPending ? "Aplicando…" : "Sí, cambiar a Gratis"}
                </Button>
                <Button
                  className="bg-white text-slate-700 ring-1 ring-slate-300 hover:bg-slate-50"
                  disabled={downgrade.isPending}
